<template>
  <jet-action-section>
    ...
    <template #content>
      <h3 v-if="$page.props.socialstream.connectedAccounts.length === 0" class="text-lg font-medium text-white">
      ...
      <div class="mt-3 ax-w-xl text-sm text-white">
        You are free to connect any social accounts to your profile and may remove any connected accounts at any time.
        If you feel any of your connected accounts have been compromised, you should disconnect them immediately and
              ...
              <template v-if="hasAccountForProvider(provider)">
                <div class="flex items-center space-x-6">
                  <button
                    ...
                    class="cursor-pointer ml-6 text-sm text-white focus:outline-none"
      ...
      <jet-dialog-modal :show="confirmingRemove" @close="confirmingRemove = false">
        ...
        <template #footer>
          ...
          <jet-button :class="{ 'opacity-25': form.processing }"
                      ...
                      class="ml-2" @click="removeConnectedAccount(accountId)">
            Remove Connected Account
          </jet-button>
        </template>
      </jet-dialog-modal>
